...
def get_posts():
    ...
    cursor.execute(query)
    result = cursor.fetchall()
    posts_df=pd.DataFrame(result,columns=["PostID","Title","Description","CreatedAtDate","Status","Topic","Type","UserID","Upvotes","Downvotes","OrganizationID","Posted_By"])
    return posts_df
...
# def fetch_other_organizations(user_id):
    
#     return other_organizations
